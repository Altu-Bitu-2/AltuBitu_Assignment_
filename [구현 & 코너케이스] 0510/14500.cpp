...
void backtracking(int row, int col, int cnt, int sum) {
    ...
    if (cnt == 4) { //기저조건 : 4개의 블럭을 탐색함
        ans = max(ans, sum);  // ans과 sum 중 최댓값 ans에 저장
        return;
    }
    ...
    for (int i = 0; i < 4; i++) {  // 상, 하, 좌, 우 4번 탐색
        ...
        if (cnt == 2) {
            ...
            board[nr][nc] = 0;  // 방문 처리
            backtracking(row, col, cnt + 1, sum + save);  // 좌표  백트래킹 탐색
            board[nr][nc] = save;
        }
        int save = board[nr][nc];   // 보드 수 저장
        board[nr][nc] = 0;  // 방문 처리
        backtracking(nr, nc, cnt + 1, sum + save);  // 상, 하, 좌, 우 백트래킹 탐색
        board[nr][nc] = save;
    }
}
...
int main() {
    ...
    board.assign(n, vector<int>(m, 0));  // 종이에 쓰여 있는 수 저장
    for (int i = 0; i < n; i++) {  // 세로 크기 n만큼 반복(행)
        for (int j = 0; j < m; j++) {  // 가로 크기 m만큼 반복(열)
            cin >> board[i][j];  // 종이에 쓰여 있는 수 입력
        }
    }
    ...
    for (int i = 0; i < n; i++) {  // 세로 크기 n만큼 반복(행)
        for (int j = 0; j < m; j++) {  // 가로 크기 m만큼 반복(열)
            int save = board[i][j];  // 좌표 수 저장
            board[i][j] = 0;  // 방문 처리, 좌표 0으로 초기화
            backtracking(i, j, 1, save); //모양 처리
            board[i][j] = save;  // 좌표 수 저장
        }
    }
