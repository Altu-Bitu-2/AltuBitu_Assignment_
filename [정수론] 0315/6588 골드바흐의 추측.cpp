...
const int SIZE = 1e6;  // 짝수 정수 최대값 1000000
...
vector<bool> isPrime() {
    ...
    for (int i = 2; i * i <= SIZE; i++) {  // i가 루트 SIZE가 될 때까지 반복
        if (!is_prime[i]) { //소수가 아니라면
            continue; 
        }
        //i가 소수라면
        for (int j = i * i; j <= SIZE; j += i) {  // j는 i*i부터 SIZE까지 i씩 키움
            is_prime[j] = false; //i의 배수 제거
        }
    }
    return is_prime;  // 소수 리턴
}
...
int main() {
    ...
    while (true) {
        //입력
        cin >> n;
        if (n == 0)  // n=0이면 (입력의 마지막 줄에 0이 주어지므로)
            break;   // 종료
