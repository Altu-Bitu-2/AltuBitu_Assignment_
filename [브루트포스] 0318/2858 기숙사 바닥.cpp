...
pair<int, int> length(int r, int b) {
    int area = r + b;  // 타일 개수의 총합
    for (int i = area; i > 0; i--) { //i = l(높이) 의 값
        if (area % i != 0) { //w 가 정수가 아니라면 넘어감
            continue;
        }
        int w = area / i;  // 기숙사 바닥의 넓이 w는 (타일개수 총합 area/기숙사 바닥의 높이 l) 
        if (r == ((i + w) * 2 - 4)) {  // 테두리 개수
            return make_pair(i, w);  // i와 w가 들어간 pair을 만들어 리턴
        }
    }
}
...
int main() {
    ...
    //연산
    pair<int, int> result = length(r, b);  // r과 b를 하나의 객체로  묶어 한번에 받음
