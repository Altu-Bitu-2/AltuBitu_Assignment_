...
int chessChange(int row, int col, vector<vector<char>>& board) {
    ...
    for (int i = 0; i < 8; i++) { //행 변화값
        for (int j = 0; j < 8; j++) { //열 변화값
            if ((i + j) % 2 == 0 && board[row + i][col + j] != 'B') {  // i+j가 짝수이고, 보드판 [행+i][열+j]인덱스에서 B가 아닐 경우
                b_cnt++;  // B로 시작하는 체스판 만들기 위한 카운트 증가
            }
            else if ((i + j) % 2 && board[row + i][col + j] != 'W') {  // i+j가 홀수이고, 보드판 [행+i][열+j]인덱스에서 W가 아닐 경우
                b_cnt++;  // B로 시작하는 체스판 만들기 위한 카운트 증가
            }
    ...
    //최솟값 리턴
    if (b_cnt > SIZE / 2) {  // 검정으로 시작하는 체스판 기준으로 계산하다 절반 이상일 경우
...
int main() {
    ...
    //입력
    cin >> n >> m;
    vector<vector<char>> board(n, vector<char>(m)); //벡터 선언, 행: n, 열: m
    for (int i = 0; i < n; i++) { // 0행부터 n-1행
        for (int j = 0; j < m; j++) {  // 0열부터 m-1열
            cin >> board[i][j];  //보드 입력
        }
    }

    //연산
    for (int i = 0; i <= n - 8; i++) {  // 행 0부터 n-8까지
        for (int j = 0; j <= m - 8; j++) {  // 열 0부터 m-8까지
