...
string findNumber(string s) {  // 30의 배수 존재하면 출력
    ...
    if (sum % 3 != 0 || !isTen) { //3의 배수가 아니거나, 10의 배수가 아니라면
        return "-1";  // 30의 배수 존재하지 않으므로 -1 출력
    }

    //가장 큰 수 만들기 위해 내림차순 정렬
    sort(s.begin(), s.end(), greater<>());
    return s;  // 30의 배수 만들어서 출력
}
...
int main() {
    string s;  // 양수 N을 string type으로 입력 받음
