...
using namespace std;
const int MOD = 10007;  // 나누는 수

int fillTile(int n) {
    vector<int> dp(n + 1, 0);  // 크기가 2xn인 직사각형을 타일로 채우는 경우의 수 저장

    if (n == 1) { //인덱스 에러 안나도록 n이 1일 경우 바로 리턴
        return 1;  // 2x1로 채우는 경우의 수 한 가지
    }

    //너비 1, 2인 타일 채우는 경우의 수 미리 초기화
    dp[1] = 1;  // n=1일 때  2x1 타일로 채우는 경우의 수 하나
    dp[2] = 3;  // n=2일 때 1x1 타일 2개, 2x2 타일 1개 총 2 경우 + 너비 1에 2x1 타일을 더한 1 경우 -> 3 경우
    for (int i = 3; i <= n; i++) {
        dp[i] = dp[i - 1] + 2 * dp[i - 2];  // (너비 1인 타일 채우는 경우의 수 = 1) * dp[n - 1]+(너비 2인 타일 채우는 경우의 수 = 2) * dp[n - 2]
