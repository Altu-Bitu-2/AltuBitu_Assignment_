...
vector<int> countAlphabet(string text) {
    ...
    for (int i = 0; i < text.size(); i++) {  // text 크기만큼 반복
        // 각 문자를 인덱스로 바꿔주기 위해 'A'의 인덱스를 빼줌
        count[text[i] - 'A']++;
    }
    return count;  // 알파벳 개수 리턴
}

string makePalindrome(vector<int>& count) {
    // 팰린드롬을 만들어 리턴하는 함수.
    string part1 = "", part2 = "", part3 = "";  // part1: part3의 대칭,  part2: 최대 1글자,  part3: part1의 대칭
    ...
    for (int i = 0; i < SIZE; i++) {  
        ...
        for (int j = 0; j < count[i] / 2; j++) {  // 남은 수의 절반만큼 반복
            part1 += (char)(i + 'A');
            part3 = (char)(i + 'A') + part3;
        }
    }

    return part1 + part2 + part3;  // 팰린드롬 만들어 리턴
}
...
int main() {
    ...
    // 연산 + 출력
    vector<int> count = countAlphabet(input);  // text에 있는 각 알파벳의 개수를 세어 리턴하는 함수에 input을 넣음.
    cout << makePalindrome(count);  //  팰린드롬을 만들어 리턴
